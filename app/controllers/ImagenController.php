...
class ImagenController extends BaseController {
    ...
    public function uploadImagenCropMultiple() {
        ...
        //if (!empty(Input::hasFile('file')) && (!empty(Input::hasFile('imagen_ampliada')))) {
            ...
            Image::make($img_ampliada)->resize(null, 800, function ($constraint) {
                $constraint->aspectRatio();
                $constraint->upsize();
            })->save($path . $filename);
            
            $img->move($path, "small_" . $filename);
            $answer = array('answer' => 'File transfer completed', 'imagen_path' => "small_" . $filename, 'imagen_ampliada' => $filename);
            echo json_encode($answer);
