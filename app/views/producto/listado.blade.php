    ...
    <div class="col-md-3 col-sm-4 col-xs-6 moduloItem">
        ...
        <div class="bandaInfoProd @if($i->producto()->nuevo()) nuevos @elseif($i->producto()->oferta()) ofertas @endif" id="Pr{{$i->producto()->id}}">
            ...
            @if(!Auth::check())
                ...
                <a class="carrito boton-presupuestar btn pull-right" href="{{URL::to('carrito/borrar/'.$i->producto()->id.'/'.$c[0].'/seccion/'.$seccion->id)}}" onclick="ShowLoading()"><i class="fa fa-check-square-o"></i>Presupuestar</a>
                @else
                    <a href="{{URL::to('carrito/agregar/'.$i->producto()->id.'/seccion/'.$seccion->id)}}" class="boton-presupuestar btn pull-right" onclick="ShowLoading()"><i class="fa fa-square-o"></i>Presupuestar</a>
